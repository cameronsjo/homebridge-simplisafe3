...
class SS3EntrySensor extends SimpliSafe3Accessory {
    ...
    async getState(callback, forceRefresh = false) {
        ...
        if (!forceRefresh) {
            const characteristic = this.service.getCharacteristic(this.api.hap.Characteristic.ContactSensorState);
        ...
        try {
            ...
            const open = sensor.status.triggered;
            const homekitState = open
