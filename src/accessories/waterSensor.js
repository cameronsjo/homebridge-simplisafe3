...
class SS3WaterSensor extends SimpliSafe3Accessory {
    ...
    setAccessory(accessory) {
        ...
        this.service
            .getCharacteristic(this.api.hap.Characteristic.LeakDetected)
    ...
    async getState(callback, forceRefresh = false) {
        ...
        if (!forceRefresh) {
            const characteristic = this.service.getCharacteristic(this.api.hap.Characteristic.LeakDetected);
        ...
        try {
            ...
            const leak = sensor.status.triggered;
            const homekitState = leak
                ? this.api.hap.Characteristic.LeakDetected.LEAK_DETECTED
                : this.api.hap.Characteristic.LeakDetected.LEAK_NOT_DETECTED;
