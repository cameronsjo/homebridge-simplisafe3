...
class SS3FreezeSensor extends SimpliSafe3Accessory {
    ...
    async getState(callback, forceRefresh = false) {
        ...
        if (!forceRefresh) {
            const characteristic = this.service.getCharacteristic(this.api.hap.Characteristic.CurrentTemperature);
        ...
        try {
            ...
            const temperature = fahrenheitToCelsius(sensor.status.temperature);
    ...
    async refreshState() {
        ...
        try {
            ...
            if (this.debug)
                this.log(`Updated current temperature, battery state for ${this.name}: ${temperature}, ${batteryLow}`);
        } catch (err) {
