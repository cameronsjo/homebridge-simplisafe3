...
class StreamingDelegate {
    constructor(ss3Camera) {
        ...
        const fps = this.cameraDetails.cameraSettings.admin.fps;
        ...
        const resolution = this.cameraDetails.cameraSettings.pictureQuality;
    ...
    async handleSnapshotRequest(request, callback) {
        ...
        if (this.ss3Camera.debug)
            this.log(`Handling camera snapshot for '${this.cameraDetails.cameraSettings.cameraName}' at ${resolution}`);
        ...
        if (!this.ss3Camera.motionIsTriggered && this.ss3Camera.supportsPrivacyShutter()) {
            ...
            const alarmSystem = await this.simplisafe.getAlarmSystem();
        ...
        try {
            const newIpAddress = await dnsLookup('media.simplisafe.com');
        ...
        jpegExtract(url)
            .then((img) => {
                if (this.ss3Camera.debug)
                    this.log(
                        `Closed '${this.cameraDetails.cameraSettings.cameraName}' snapshot request with ${Math.round(img.length / 1000)}kB image`
                    );
                callback(undefined, img);
            })
            .catch((err) => {
                this.log.error(
                    'An error occurred while making snapshot request:',
    ...
    handlePrivacyShutterClosedSnapshotRequest(callback) {
        if (this.ss3Camera.debug)
            this.log(
                `Camera snapshot request ignored, '${this.cameraDetails.cameraSettings.cameraName}' privacy shutter closed`
            );
    ...
    handleUnsupportedCameraSnapshotRequest(callback) {
        if (this.ss3Camera.debug)
            this.log(
                `Camera snapshot request ignored, '${this.cameraDetails.cameraSettings.cameraName}' is not supported`
            );
    ...
    async handleStreamRequest(request, callback) {
        ...
        if (this.ss3Camera.isUnsupported()) {
            const err = new Error(`Camera ${this.ss3Camera.name} is unsupported`);
            this.log.error(err);
            callback(err);
            return;
        }
        ...
        if (sessionId) {
            ...
            if (request.type === 'start') {
                ...
                if (sessionInfo) {
                    ...
                    try {
                        ...
                        if (!this.serverIpAddress) {
                            delete this.pendingSessions[sessionIdentifier];
                            this.log.error(
                                'Camera stream request failed, could not resolve hostname for media.simplisafe.com',
                                err
                            );
                            callback(err);
                            return;
                    ...
                    let sourceArgs = [
                        ...
                        [
                            '-i',
                            `https://${this.serverIpAddress}/v1/${this.ss3Camera.cameraDetails.uuid}/flv?x=${width}&audioEncoding=AAC`
                        ]
                    ];
                    ...
                    if (isDocker() && (!this.ss3Camera.cameraOptions || !this.ss3Camera.cameraOptions.ffmpegPath)) {
                        // if docker and no custom binary specified
                        if (this.ss3Camera.debug)
                            this.log(
                                'Detected running in docker container with bundled binary, limiting to 720px wide'
                            );
                        width = Math.min(width, 720);
                        const vFilterArg = videoArgs.find((arg) => arg[0] === '-vf');
                        vFilterArg[1] = `scale=${width}:-2`;
                    }

                    if (request.audio && request.audio.codec === 'OPUS') {
                        // Request is for OPUS codec, serve that
                        const iArg = sourceArgs.find((arg) => arg[0] === '-i');
                        iArg[1] = iArg[1].replace('&audioEncoding=AAC', '');
                        const aCodecArg = audioArgs.find((arg) => arg[0] === '-acodec');
                        aCodecArg[1] = 'libopus';
                        const profileArg = audioArgs.find((arg) => arg[0] === '-profile:a');
                    ...
                    if (this.ss3Camera.cameraOptions) {
                        if (this.ss3Camera.cameraOptions.enableHwaccelRpi) {
                            ...
                            const vCodecArg = videoArgs.find((arg) => arg[0] === '-vcodec');
                        ...
                        if (this.ss3Camera.cameraOptions.sourceOptions) {
                            const options =
                                typeof this.ss3Camera.cameraOptions.sourceOptions === 'string'
                                    ? Object.fromEntries(
                                          this.ss3Camera.cameraOptions.sourceOptions
                                              .split('-')
                                              .filter((x) => x)
                                              .map((arg) => `-${arg}`)
                            ...
                            for (const key in options) {
                                ...
                                const existingArg = sourceArgs.find((arg) => arg[0] === key);
                                if (existingArg) {
                                    if (value === false) {
                        ...
                        if (this.ss3Camera.cameraOptions.videoOptions) {
                            const options =
                                typeof this.ss3Camera.cameraOptions.videoOptions === 'string'
                                    ? Object.fromEntries(
                                          this.ss3Camera.cameraOptions.videoOptions
                                              .split('-')
                                              .filter((x) => x)
                                              .map((arg) => `-${arg}`)
                                              .map((a) => a.split(' ').filter((x) => x))
                            ...
                            for (const key in options) {
                                const value = options[key];
                                const existingArg = videoArgs.find((arg) => arg[0] === key);
                                if (existingArg) {
                                    if (value === false) {
                        ...
                        if (this.ss3Camera.cameraOptions.audioOptions) {
                            const options =
                                typeof this.ss3Camera.cameraOptions.audioOptions === 'string'
                                    ? Object.fromEntries(
                                          this.ss3Camera.cameraOptions.audioOptions
                                              .split('-')
                                              .filter((x) => x)
                                              .map((arg) => `-${arg}`)
                                              .map((a) => a.split(' ').filter((x) => x))
                            ...
                            for (const key in options) {
                                const value = options[key];
                                const existingArg = audioArgs.find((arg) => arg[0] === key);
                                if (existingArg) {
                                    if (value === false) {
                    ...
                    const source = [].concat(
                        ...sourceArgs.map((arg) => arg.map((a) => (typeof a === 'string' ? a.trim() : a)))
                    );
                    const video = [].concat(
                        ...videoArgs.map((arg) => arg.map((a) => (typeof a === 'string' ? a.trim() : a)))
                    );
                    const audio = [].concat(
                        ...audioArgs.map((arg) => arg.map((a) => (typeof a === 'string' ? a.trim() : a)))
                    );

                    try {
                        const cmd = spawn(this.ss3Camera.ffmpegPath, [...source, ...video, ...audio], {
                            env: process.env
                        });
                        ...
                        if (this.ss3Camera.debug) {
                            ...
                            this.log(
                                [this.ss3Camera.ffmpegPath, source.join(' '), video.join(' '), audio.join(' ')].join(
                                    ' '
                                )
                            );
                        }
                ...
                const cmd = this.ongoingSessions[sessionIdentifier];
                try {
                    if (cmd) {
                        cmd.kill('SIGKILL');
                    }
